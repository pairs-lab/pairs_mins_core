...
tmux send-keys -t "$P" "$SETUP; "'waitForRos; rosparam set use_sim_time true; roslaunch pairs_mins_core rosbag_pico_t265.launch' Enter
...
tmux send-keys -t "$P" "$SETUP; "'waitForControl; rviz -d `rospack find pairs_mins_core`/rviz/rosbag_uav46_pico_t265.rviz' Enter
